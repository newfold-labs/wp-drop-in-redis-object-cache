...
function wp_cache_add_non_persistent_groups( $groups ) {
    ...
    /**
     * Bluehost Redis Object Cache Implementation
     *
     * Handles all WordPress object cache operations using Redis as the backend.
     * Provides support for multiple Redis clients and advanced configurations.
     */
    #[AllowDynamicProperties]
    class WP_Object_Cache {

    /**
     * Bluehost plugin directory slugs used to locate Predis/Credis in vendor.
     * Must match: installer name (bluehost-wordpress-plugin) and package slug (wp-plugin-bluehost).
     * Do not change without updating the Bluehost plugin and deployment paths.
     *
     * @var string[]
     */
    private static $bluehost_plugin_dir_slugs = [ 'bluehost-wordpress-plugin', 'wp-plugin-bluehost' ];
    ...
    /**
     * Get the plugins base path (WP_PLUGIN_DIR or wp-content/plugins).
     *
     * @return string Non-empty path to plugins directory.
     */
    private static function get_plugins_base_path() {
        if ( defined( 'WP_PLUGIN_DIR' ) && WP_PLUGIN_DIR !== '' ) {
            return WP_PLUGIN_DIR;
        }
        return defined( 'WP_CONTENT_DIR' ) ? WP_CONTENT_DIR . '/plugins' : '';
    }

    /**
     * Get full paths to each candidate Bluehost plugin directory.
     * Used to locate Predis and Credis under plugin vendor.
     *
     * @return string[] List of absolute paths (may include non-existent dirs).
     */
    private static function get_bluehost_plugin_base_paths() {
        $base = self::get_plugins_base_path();
        if ( $base === '' ) {
            return [];
        }
        $paths = [];
        foreach ( self::$bluehost_plugin_dir_slugs as $slug ) {
            $paths[] = $base . '/' . $slug;
        }
        return $paths;
    }
    ...
    /**
     * Initialize the Bluehost Redis object cache.
     *
     * Sets up connection to Redis server and configures cache groups and behavior.
     *
     * @param bool $fail_gracefully If true, falls back to in-memory cache on Redis failure.
     */
    public function __construct( $fail_gracefully = false ) {
        ...
        $this->global_groups[] = 'bluehost-cache';
    ...
    /**
     * Build Redis connection parameters from WordPress configuration constants.
     *
     * Merges default values with user-defined constants from wp-config.php.
     *
     * @return array Connection parameters array.
     */
    ...
    /**
     * Establish Redis connection using PhpRedis PECL extension.
     *
     * PhpRedis provides native PHP extension for optimal performance.
     *
    ...
    /**
     * Establish Redis connection using Relay extension.
     *
     * Relay provides in-memory caching with Redis backend.
     *
     * @param  array $parameters Connection parameters.
     * @return void
     */
    ...
    /**
     * Establish Redis connection using Predis client.
     *
     * Predis is a pure PHP Redis client library provided by Bluehost Plugin.
     *
     * @param  array $parameters Connection parameters.
     * @throws \Exception If Predis library cannot be located.
     * @return void
     */
    protected function connect_using_predis( $parameters ) {
        ...
        if ( ! class_exists( 'Predis\Client' ) ) {
            $plugin_base_paths = self::get_bluehost_plugin_base_paths();

            // Try Bluehost plugin vendor: plugin root vendor first, then performance module vendor
            $predis_paths = [];
            foreach ( $plugin_base_paths as $plugin_base ) {
                if ( $plugin_base === '' ) {
                    continue;
                }
            ...
            if ( ! $loaded && ! class_exists( 'Predis\Client' ) ) {
                foreach ( $plugin_base_paths as $plugin_base ) {
                    ...
                    $autoloader_path = $plugin_base . '/vendor/autoload.php';
            ...
            if ( ! $loaded && ! class_exists( 'Predis\Client' ) ) {
                throw new Exception(
                    'Predis library not found. Please ensure the Bluehost Plugin is installed with the Performance module enabled, or delete the object-cache.php file.'
                );
            }
        }
    ...
    /**
     * Establish Redis connection using Credis library (legacy support).
     *
     * Note: Credis support is deprecated. Use Predis or PhpRedis instead.
     *
     * @param  array $parameters Connection parameters.
     * @throws \Exception If Credis library cannot be located or configuration is invalid.
     * @return void
     */
    protected function connect_using_credis( $parameters ) {
        trigger_error( 'Credis support is deprecated and will be removed in a future version', E_USER_DEPRECATED );

        $client = 'Credis';

        // Locate Credis library from Bluehost Plugin vendor (legacy support only)
        $creds_path = null;
        foreach ( self::get_bluehost_plugin_base_paths() as $plugin_base ) {
            if ( $plugin_base === '' ) {
                continue;
            }
    ...
    /**
     * Retrieve Redis server information and version.
     *
     * Fetches server metadata for diagnostics and version checking.
     *
     * @return void
     */
    ...
    protected function add_multiple_at_once( array $data, $group = 'default', $expire = 0 ) {
        ...
        foreach ( $data as $key => $value ) {
             ...
             */
            $expire = apply_filters( 'bluehost_redis_cache_expiration', $expire, $key, $group, $orig_exp );
    ...
    protected function add_or_replace( $add, $key, $value, $group = 'default', $expiration = 0 ) {
        ...
        if ( ! $this->is_ignored_group( $san_group ) && $this->redis_status() ) {
            try {
                 ...
                 * @param mixed  $orig_exp   The original expiration value before validation.
                 */
                $expiration = apply_filters( 'bluehost_redis_cache_expiration', $expiration, $key, $group, $orig_exp );
                $start_time = microtime( true );
...
LUA;
    ...
    protected function set_multiple_at_once( array $data, $group = 'default', $expiration = 0 ) {
        ...
        foreach ( $data as $key => $value ) {
             ...
             * @param mixed  $orig_exp   The original expiration value before validation.
             */
            $expiration = $expirations[ $key ] = apply_filters( 'bluehost_redis_cache_expiration', $expiration, $key, $group, $orig_exp );
    ...
    public function add_non_persistent_groups( $groups ) {
         ...
         */
        $groups = apply_filters( 'bluehost_redis_cache_add_non_persistent_groups', (array) $groups );
    ...
    protected function show_error_and_die( Exception $exception ) {
        ...
        $message = '<h1>' . __( 'Error establishing a Redis connection', 'wp-plugin-bluehost' ) . "</h1>\n";
        ...
        if ( $verbose ) {
            ...
            $message .= '<p>' . sprintf(
                ...
                __( 'WordPress is unable to establish a connection to Redis. This means that the connection information in your %s file are incorrect, or that the Redis server is not reachable.', 'wp-plugin-bluehost' ),
                '<code>wp-config.php</code>'
            ) . "</p>\n";

            $message .= "<ul>\n";
            $message .= '<li>' . __( 'Is the correct Redis host and port set?', 'wp-plugin-bluehost' ) . "</li>\n";
            $message .= '<li>' . __( 'Is the Redis server running?', 'wp-plugin-bluehost' ) . "</li>\n";
            $message .= '<li>' . __( 'Is the Bluehost Plugin installed with the Performance module enabled?', 'wp-plugin-bluehost' ) . "</li>\n";
            $message .= "</ul>\n";
        }

        $message .= '<p>' . sprintf(
            // translators: %1$s = Formatted object-cache.php file name, %2$s = Formatted wp-content directory name.
            __( 'To disable Redis, delete the %1$s file in the %2$s directory.', 'wp-plugin-bluehost' ),
